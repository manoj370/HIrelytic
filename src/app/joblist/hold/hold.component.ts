...
export class HoldComponent implements OnInit, OnDestroy {
  ...
  onBtNext() {
    ...
    // Previous Code
    // this._jobpostingService.getjoblistBystatus(sessionStorage.getItem('tenantId'), this.pageid, this.rowid,'ONHOLD').subscribe(res => {
    //   this.rowData = res;
  ...
  downloadHoldJobReportPdf() {
    ...
    if (this.userRoles == 'ADMIN') {
      this.ReportsService.downloadHoldJobReportPdf().subscribe(res => {
        let blob = res;
        let a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'holdJobs.pdf' + ' ' + this.datee + '.pdf';
      ...
      this.ReportsService.downloadHoldJobReportPdf().subscribe(res => {
        ...
        a.href = URL.createObjectURL(blob);
        a.download = 'holdJobsPdfuser.pdf' + '' + this.datee + '.pdf';
        document.body.appendChild(a);
        a.click();
      });
    }
  ...
  downloadHoldExcelReportJobsReport() {
    ...
    console.log(this.datee);
    if (this.userRoles === 'ADMIN') {
      this.ReportsService.downloadHoldExcelReportJobsReport()
        .pipe(takeUntil(this.subscribe))
        .subscribe(res => {
          let blob = res;
          let a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'holdJobs.xls' + '' + this.datee + '.xls';
          document.body.appendChild(a);
          a.click();
        });
    } else {
