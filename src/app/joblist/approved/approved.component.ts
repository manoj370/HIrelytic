...
export class ApprovedComponent implements OnInit, OnDestroy {
    ...
    privileges.forEach(element => {
      ...
      this.privileges.forEach((elementt: any) => {
        ...
        // if ((this.authority === 'JOBS POSTED BY ME') && (sessionStorage.getItem('userRoles') !== 'ADMIN')) {
          if ((this.authority === 'JOBS POSTED BY ME') && (sessionStorage.getItem('userRoles') !== 'ADMIN')) {
          this._jobpostingService.accountJobsStaus(this.pageid, this.rowid, 'ASSIGNED')
          ...
          this._jobpostingService.getjoblistBystatus(sessionStorage.getItem('tenantId'), this.pageid, this.rowid, 'ASSIGNED')
            .pipe(takeUntil(this.subscribe))
            .subscribe(Data => {
              // Open
              console.log(Data);
          ...
          this._jobpostingService.jobsByStatus(this.pageid, this.rowid, 'ASSIGNED')
            .pipe(takeUntil(this.subscribe))
            .subscribe(Data => {
              //Open
              console.log(Data);
  ...
  onBtNext() {
    ...
    // Previous Code
    // this._jobpostingService.getjoblistBystatus(sessionStorage.getItem('tenantId'), this.pageid, this.rowid, 'ASSIGNED').subscribe(res => {
    //   this.rowData = res;
  ...
  downloadAssignedJobReportPdf() {
    ...
    if (this.userRoles == 'ADMIN') {
      this.ReportsService.downloadAssignedJobReportPdf().subscribe(res => {
        let blob = res;
        let a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'assignedJobsPdf.pdf' + ' ' + this.datee + '.pdf';
      ...
      this.ReportsService.downloadAssignedJobReportPdf().subscribe(res => {
        ...
        a.href = URL.createObjectURL(blob);
        a.download = 'assignedJobsPdfuser.pdf' + '' + this.datee + '.pdf';
        document.body.appendChild(a);
        a.click();
      });
    }
  }
  downloadAssignedExcelReportJobsReport() {
    let date: Date = new Date();
    this.datee = date.getFullYear().toString() + '-' + ('0' + (date.getMonth() + 1).toString()).slice(-2) + '-' + ('0' + (date.getDate()).toString()).slice(-2);
    console.log(this.datee);
    if (this.userRoles === 'ADMIN') {
      this.ReportsService.downloadAssignedJobReportExcel()
        .pipe(takeUntil(this.subscribe))
        .subscribe(res => {
          let blob = res;
          let a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'AssignedJobsExcel.xls' + '' + this.datee + '.xls';
          document.body.appendChild(a);
          a.click();
      ...
      this.ReportsService.downloadAssignedJobReportExcel()
        ...
        .subscribe(res => {
          ...
          const a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'assignedJobsExceluser.pdf' + '' + this.datee + '.xls';
          document.body.appendChild(a);
          a.click();
        });
    }
