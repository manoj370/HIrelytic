...
export class RejectedComponent implements OnInit, OnDestroy {
  ...
  datee: string;
  userRoles: any;
  rowClassRules: { 'sick-days-warning': (params: any) => boolean; };
  rowHeight: number;
  privileges: any;
  authority: any;
    ...
    privileges.forEach(element => {
      ...
      this.privileges.forEach((elementt: any) => {
        ...
        if ((this.authority === 'JOBS POSTED BY ME') && (sessionStorage.getItem('userRoles') !== 'ADMIN')) {
          this._jobpostingService.accountJobsStaus(this.pageid, this.rowid, 'REJECTED')
          ...
          this._jobpostingService.getjoblistBystatus(sessionStorage.getItem('tenantId'), this.pageid, this.rowid, 'REJECTED')
            .pipe(takeUntil(this.subscribe))
            .subscribe(Data => {
              //Open
              console.log(Data);
          ...
          this._jobpostingService.jobsByStatus(this.pageid, this.rowid, 'REJECTED')
            .pipe(takeUntil(this.subscribe))
            .subscribe(Data => {
              //Open
              console.log(Data);
  ...
  onBtNext() {
    ...
    if ((this.authority === 'JOBS POSTED BY ME') && (sessionStorage.getItem('userRoles') !== 'ADMIN')) {
      this._jobpostingService.accountJobsStaus(this.pageid, this.rowid, 'ONHOLD')
        .pipe(takeUntil(this.subscribe))
        .subscribe(Data => {
          //Open
          console.log(Data);
      ...
      this._jobpostingService.getjoblistBystatus(sessionStorage.getItem('tenantId'), this.pageid, this.rowid, 'ONHOLD')
        .pipe(takeUntil(this.subscribe))
        .subscribe(Data => {
          //Open
          console.log(Data);
      ...
      this._jobpostingService.jobsByStatus(this.pageid, this.rowid, 'ONHOLD')
        .pipe(takeUntil(this.subscribe))
        .subscribe(Data => {
          //Open
          console.log(Data);
    ...
    // Previous Code
    // this._jobpostingService.getjoblistBystatus(sessionStorage.getItem('tenantId'), this.pageid, this.rowid,'REJECTED').subscribe(res => {
    //   this.rowData = res;
  ...
  downloadRejectJobReportPdf() {
    ...
    if (this.userRoles == 'ADMIN') {
      this.ReportsService.downloadRejectJobReportPdf()
        .pipe(takeUntil(this.subscribe))
        .subscribe(res => {
          let blob = res;
          let a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'rejectedJobs.pdf' + ' ' + this.datee + '.pdf';
      ...
      this.ReportsService.downloadRejectJobReportPdf()
        ...
        .subscribe(res => {
          ...
          a.href = URL.createObjectURL(blob);
          a.download = 'rejectJobsPdfuser.pdf' + '' + this.datee + '.pdf';
          document.body.appendChild(a);
          a.click();
        });
    }
  }
  downloadRejectExcelReportJobsReport() {
    let date: Date = new Date();
    this.datee = date.getFullYear().toString()
      + '-' + ('0' + (date.getMonth() + 1).toString()).slice(-2) +
      '-' + ('0' + (date.getDate()).toString()).slice(-2);
    console.log(this.datee);
    if (this.userRoles == 'ADMIN') {
      this.ReportsService.downloadRejectJobReportExcel()
        .pipe(takeUntil(this.subscribe))
        .subscribe(res => {
          let blob = res;
          let a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'rejectedJobs.xls' + ' ' + this.datee + '.xls';
          document.body.appendChild(a);
          a.click();
      ...
      this.ReportsService.downloadRejectJobReportExcel()
        ...
        .subscribe(res => {
          ...
          const a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'rejectJobsExceluser.pdf' + '' + this.datee + '.xls';
          document.body.appendChild(a);
          a.click();
        });
    }
